# rory-os — the shared-brain control plane
...
```bash
cd rory/os
node cli/rory.js demo:e2e     # mocked end-to-end workflow (safe, in-memory)
node cli/rory.js init         # create the operational DB + load agent manifests
node cli/rory.js status       # one-glance status
node cli/rory.js board        # today's board meeting (writes MD + JSON)
node --test                     # run the 48-test suite
```

## Layout
See `../../docs/architecture/overview.md`. Every service is a small module under
its own directory; `core/context.js` wires them; `cli/rory.js` is the entry
point.
...
## Configuration
- `RORY_DB_PATH` — DB location (default `rory/data/rory.db`).
- `rory/os/config/model-profiles.json` — model profiles; concrete model ids
