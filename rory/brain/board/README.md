...
at `rory/data/board/YYYY-MM-DD.json`.

## How it works
`rory/os/board/meeting.js` reads the **append-only event ledger** for the
...
```bash
cd rory/os
node cli/rory.js board            # writes the two files; refuses to overwrite
node cli/rory.js board --regenerate
```
