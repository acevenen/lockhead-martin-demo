...
- [ ] *(optional)* **ElevenLabs** — for the cinematic RORY voice instead of
...
- [ ] Run **`roryd/install/windows/enable-wol.ps1`** as admin, then set the
...
- [ ] Install Python, `pip install -r roryd/requirements.txt`, run
  **`install-roryd.ps1`**, and turn **mic permission** on
  (Settings → Privacy → Microphone, incl. "let desktop apps access…").

## 3. MacBook Air (you)

- [ ] Install **Tailscale** and log in.
- [ ] Install the RORY daemon LaunchAgent (`roryd/install/macos/…`), edit the
...
- [ ] **roryd bridge path** (no HA): run `python3 -m roryd.wol serve` on any
  always-on wired device, set `RORY_WOL_TARGETS` + `RORY_WOL_TOKEN`, and
...
- Wiring **`roryd` ↔ Node 001**, memory indexing of this repo first, and the
  model-routing rules.
