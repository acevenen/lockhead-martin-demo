...
function cmdHelp() {
  out(`jarvis-os — local-first control plane
...
Env: JARVIS_DB_PATH overrides the database location.`);
}

function withCtx(fn, opts = {}) {
  const path = process.env.JARVIS_DB_PATH;
...
function cmdInit() {
  withCtx((ctx) => {
    ...
    out('Next: `node cli/jarvis.js demo:e2e` for a full dry run, or `status`.');
  });
}
