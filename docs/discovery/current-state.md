...
- **Git:** active repo, branch `claude/centlabs-jarvisd-daemon` at time of scan.
  Working tree clean. Recent history: glasses bridge + Node 001 data plane,
  CentLabs connect checklist, jarvisd daemon.
- **Nested git repos:** none (verified — no nested `.git`). The Jarvis OS is
...
| JARVIS command HUD | `jarvis/index.html`, `jarvis/state.json`, `jarvis/ledger.json` | Voice + dashboard surface; `state.json` is the live HUD state, `ledger.json` the daily-improvement log |
| Knowledge base | `jarvis/knowledge/{values,business}.md` | Owner's constitution seed + business facts (mostly placeholders) |
...
| Operating protocol | `CLAUDE.md` | The JARVIS chief-of-staff protocol; defines the implicit orchestrator role |
| Sentinel | separate repo (`acevenen/sentinel`), cloned at `/workspace/sentinel` | Go security CLI; **not** part of this repo |
| jarvisd | `jarvisd/` | Python always-on voice daemon (wake word, brief, Wake-on-LAN) |
...
- Existing code is heterogeneous by design: browser JS (demos), Python
  (`jarvisd`), Go (Sentinel, separate repo), Markdown+JSON (the brain/HUD).
...
1. **Integrate, don't compete.** The Jarvis OS is added under the existing
   `jarvis/` tree (`jarvis/brain/`, `jarvis/os/`, `jarvis/data/`) so it sits
   beside the existing HUD instead of forming a competing nested app. The
   existing `jarvis/state.json` and `jarvis/index.html` are left untouched.
