<#
    Install the JARVIS daemon as a per-user autostart task on Windows 11.
    ...
    * RestartCount so a crash is recovered automatically.
    ...
    Run in an ordinary (non-admin) PowerShell:
        powershell -ExecutionPolicy Bypass -File install-jarvisd.ps1

    Uninstall:  Unregister-ScheduledTask -TaskName "JARVIS Daemon" -Confirm:$false
#>
...
$JarvisdDir = Join-Path $RepoRoot "jarvisd"
...
$Action = New-ScheduledTaskAction -Execute $PythonW `
    -Argument "-m jarvisd.daemon" -WorkingDirectory $JarvisdDir
...
Register-ScheduledTask -TaskName "JARVIS Daemon" `
    -Action $Action -Trigger $TriggerLogon -Settings $Settings -Principal $Principal `
    -Description "CentLabs JARVIS always-on voice daemon" -Force | Out-Null

# JARVIS_HOME lets the daemon find the repo regardless of the working dir.
[Environment]::SetEnvironmentVariable("JARVIS_HOME", $RepoRoot, "User")

Write-Host ""
Write-Host "Installed. The daemon starts at your next log on."
Write-Host "Start it now with:  Start-ScheduledTask -TaskName 'JARVIS Daemon'"
Write-Host ""
