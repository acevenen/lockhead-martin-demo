"""Configuration and filesystem layout for the JARVIS daemon.
...
def _repo_root() -> Path:
    ...
    jarvisd/jarvisd/config.py -> repo root is two levels up. An explicit
    JARVIS_HOME wins, so the daemon can run from an install location that is
...
class Config:
    ...
    def state_file(self) -> Path:
        return self.root / "jarvis" / "state.json"

    @property
    def ledger_file(self) -> Path:
        return self.root / "jarvis" / "ledger.json"

    @property
    def hud_file(self) -> Path:
        return self.root / "jarvis" / "index.html"

    @property
    def log_file(self) -> Path:
        return self.log_dir / "jarvisd.log"
    ...
    def log_dir(self) -> Path:
        ...
        if os.name == "nt":
            ...
            return base / "jarvisd"
        return Path.home() / ".local" / "state" / "jarvisd"
    ...
    def honorific(self) -> str:
        return os.environ.get("JARVIS_HONORIFIC", "sir").strip() or "sir"
...
def load() -> Config:
    ...
    return Config(
        root=cfg.root,
        wake_model=os.environ.get("JARVIS_WAKE_MODEL", cfg.wake_model).strip() or cfg.wake_model,
        wake_threshold=_float("JARVIS_WAKE_THRESHOLD", cfg.wake_threshold),
        command_seconds=_float("JARVIS_COMMAND_SECONDS", cfg.command_seconds),
        silence_seconds=_float("JARVIS_SILENCE_SECONDS", cfg.silence_seconds),
        sample_rate=cfg.sample_rate,
        frame_size=cfg.frame_size,
        stt_model=os.environ.get("JARVIS_STT_MODEL", cfg.stt_model).strip() or cfg.stt_model,
        eleven_voice=os.environ.get("JARVIS_VOICE_ID", cfg.eleven_voice).strip() or cfg.eleven_voice,
        http_port=_int("JARVIS_HTTP_PORT", cfg.http_port),
        open_hud_on_start=os.environ.get("JARVIS_OPEN_HUD", "1").strip() != "0",
    )
