    ...
    def text_mode(self) -> None:
        ...
        if not sys.stdin or not sys.stdin.isatty():
            ...
            while not self._stop.wait(3600):
                pass
            return
        print("JARVIS text mode — type a command, or 'quit'.")
...
def make_server(jarvis: Jarvis) -> ThreadingHTTPServer:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
            if self.path.rstrip("/") in ("/brief", ""):
                self._send(jarvis.refresh_brief().to_dict())
            elif self.path.rstrip("/") == "/state":
                try:
                    self._send(json.loads(Path(jarvis.cfg.state_file).read_text("utf-8")))
                except Exception:
                    self._send({"error": "state unavailable"}, 503)
            elif self.path.rstrip("/") == "/health":
                self._send({
                    "ok": True,
                    "wake": jarvis.wake.available,
                    "stt": jarvis.ears.available,
                    "voice": jarvis.voice.available,
                })
            else:
    ...
    return ThreadingHTTPServer(("127.0.0.1", jarvis.cfg.http_port), Handler)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="jarvisd", description="Always-on JARVIS voice daemon"
    )
    ...
    if args.check:
        print(json.dumps({
            "wake_word": jarvis.wake.available,
            "speech_to_text": jarvis.ears.available,
            "voice_out": jarvis.voice.available,
    ...
    if args.once:
        print(jarvis.respond(args.once))
        return 0

    server = None
    if not args.no_serve:
        try:
            server = make_server(jarvis)
