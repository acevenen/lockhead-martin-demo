# jarvisd — the always-on JARVIS daemon

Siri-on-your-PC. It starts when the machine comes up, listens for **"Hey
Jarvis,"** has the brief ready the moment you wake it, and answers with the same
...
```sh
python3 -m jarvisd.daemon --check     # what's available on this machine
python3 -m jarvisd.daemon --brief     # print today's brief and exit
python3 -m jarvisd.daemon --once "hey jarvis, what did i miss"
python3 -m jarvisd.daemon             # run for real: listen + serve + open HUD
```
...
`install/macos/com.centlabs.jarvisd.plist`, then:

```sh
cp install/macos/com.centlabs.jarvisd.plist ~/Library/LaunchAgents/
launchctl load ~/Library/LaunchAgents/com.centlabs.jarvisd.plist
```
...
```powershell
powershell -ExecutionPolicy Bypass -File install\windows\install-jarvisd.ps1
```
...
**Linux (future Node migration):** `install/linux/jarvisd.service`, a
...
```sh
export JARVIS_WOL_TARGETS="node001=AA:BB:CC:DD:EE:FF/192.168.1.255"
export JARVIS_WOL_TOKEN="a-long-random-secret"
python3 -m jarvisd.wol serve            # POST /wake/node001 with X-Token
python3 -m jarvisd.wol send AA:BB:CC:DD:EE:FF --broadcast 192.168.1.255
```
...
The daemon reads the same `jarvis/state.json` the HUD and the Claude Code agents
use, so the spoken brief and the screen can never disagree. It also reads
`jarvis/ledger.json`, the **daily improvement ledger** (`jarvisd/jarvisd/ledger.py`):
