...
log = logging.getLogger("roryd.wol")
...
def serve(port: int = 8792) -> ThreadingHTTPServer:  # pragma: no cover - network
    ...
    POST /wake/<name>  with header  X-Token: <RORY_WOL_TOKEN>
    ...
    """
    secret = os.environ.get("RORY_WOL_TOKEN", "").strip()
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802
            if not secret:
                self._reply(503, {"error": "RORY_WOL_TOKEN not set; endpoint disabled"})
                return
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="roryd-wol", description="Wake-on-LAN sender + bridge")
